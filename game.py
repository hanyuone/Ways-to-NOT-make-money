...
master = Tk()
master.title("Ways To NOT Make Money")
...
def bugfixer():
    ...
    while game_state.mps > (game_state.autoclick + game_state.printmoney * 15 + game_state.counterfeit * 321 +
        ...
        if game_state.mps - 2015 >= (game_state.autoclick + game_state.printmoney * 15 + game_state.counterfeit * 321 +
            ...
            game_state.bankprice = round(175000 * (math.pow(1.1, game_state.bankheist2)), 2)
            bankpricechoice()
            ...
            game_state.shareprice = round(42000 * (math.pow(1.4, game_state.sharecrash2)), 2)
            sharepricechoice()
            ...
            game_state.counterprice = round(9001 * (math.pow(1.3, game_state.counterfeit2)), 2)
            counterpricechoice()
            ...
            counterpricechoice()
            game_state.printprice = round(375 * (math.pow(1.25, game_state.printmoney2)), 2)
            printpricechoice()

        else:
            game_state.autoclick += (1 + game_state.upgcheck1h1 * 2 + game_state.upgcheck1h2 * 18)
            game_state.autoclick2 += 1
            game_state.autoprice = round(20 * (math.pow(1.15, game_state.autoclick2)), 2)
            autopricechoice()


def deductionx(name, purchase_msg, choice_fn):
    log('deductionx invoked', name)
    global game_state
    if not game_state.can_increment(name):
        master.bell()
        messagetkinter.set(cannotafford)

    else:
        game_state.increment(name)
        mpstkinter.set("MPS: %s" % game_state.mps)
        messagebox(purchase_msg)
        choice_fn()
...
def deduction2():
    deductionx('printprice', 'Money Printer purchased', printpricechoice)


def deduction3():
    deductionx('counterprice', 'Counterfeit Company purchased', counterpricechoice)


def deduction4():
    deductionx('shareprice', 'Sharemarket Crash purchased', sharepricechoice)


def deduction5():
    deductionx('bankprice', 'Bank Heist purchased', sharepricechoice)
...
def normal_upgrade(button_frame, tup):
    ...
    (button_label, alt_func, cost, state_dict_key_check, state_dict_key_mod, done_attr, incr_ratio, mps_incr_ratio) = \
        tup
    ...
    if alt_func:
        ...
        if game_state.money < cost or getattr(game_state, state_dict_key_check) == 0:
            master.bell()
            messagetkinter.set(norequirements)
        else:
...
def goldupgrade():
    ...
    if randint(1, 77) == 1:
        ...
        game_state.inc *= 77
        mpstkinter.set("MPS: %s" % game_state.mps)
        messagebox("Gold: *77 profits for 77 seconds!")
        master.after(77000, goldmpsstop)
    else:
        game_state.money += int(game_state.mps * 50)
        messagebox("Gold: Money + (50 * MPS)!")


def goldmpsstop():
    game_state.mps *= (1 / 77.0)
    game_state.inc *= (1 / 77.0)
    messagebox("Gold finished.")
    mpstkinter.set("MPS: %s" % game_state.mps)
...
def hideupgrades():
    global exitupgrades, game_state, bf
...
def main():
    ...
    global incbutton1, incbutton2, incbutton3, incbutton4, incbutton5, upgrades, clickbutton, lottobutton, \
        messagetkinter, multiplier, moneytkinter, mpstkinter, totalclicksvar, timevar, totalspentvar, \
        inctkinter, lottoprice, messagetkinter
    ...
    messagelabel = Label(master, textvariable=messagetkinter)
    messagelabel.grid(row=0, column=1)
    ...
    incbutton1 = frames.ItemFrame(master, 'Auto-Clicker ($%s)' % format_price(game_state.autoprice),
                                  '$%d/s, %d owned' % (game_state.autoclick, game_state.autoclick2), deduction1)
    incbutton1.grid(row=1, column=0, sticky=N+E+W+S)

    incbutton2 = frames.ItemFrame(master, 'Money Printer ($%s)' % format_price(game_state.printprice),
                                  '$%d/s, %d owned' % (game_state.printmoney * 15, game_state.printmoney2),
                                  deduction2)
    incbutton2.grid(row=2, column=0, sticky=N+E+W+S)

    incbutton3 = frames.ItemFrame(master, 'Counterfeit Company ($%s)' % format_price(game_state.counterprice),
                                  '$%d/s, %d owned' % (game_state.counterfeit * 221, game_state.counterfeit2),
                                  deduction3)
    incbutton3.grid(row=3, column=0, sticky=N+E+W+S)

    incbutton4 = frames.ItemFrame(master, 'Sharemarket Crash ($%s)' % format_price(game_state.shareprice),
                                  '$%d/s, %d owned' % (game_state.sharecrash * 969, game_state.sharecrash2),
                                  deduction4)
    incbutton4.grid(row=4, column=0, sticky=N+E+W+S)

    incbutton5 = frames.ItemFrame(master, 'Bank Heist ($%s)' % format_price(game_state.bankprice),
                                  '$%d/s, %d owned' % (game_state.bankheist * 2015, game_state.bankheist2), deduction5)
