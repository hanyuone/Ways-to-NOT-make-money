...
class GameState:
    ...
    def increment(self, name):
        ...
        if name == 'autoprice':
            ...
            self.autoprice = 20 * (math.pow(1.15, self.autoclick2))
    ...
    def bump_multiplier(self):
        ...
        for a in range(self.multiplier):
            self.autoprice += float(20 * math.pow(1.15, (self.autoclick2 + a)))
            self.printprice += float(375 * math.pow(1.25, (self.printmoney2 + a)))
            self.counterprice += float(9001 * math.pow(1.3,  (self.counterfeit2 + a)))
            self.shareprice += float(42000 * math.pow(1.4,  (self.sharecrash2 + a)))
            self.bankprice += float(175000 * math.pow(1.1,  (self.bankheist2 + a)))

    def get_totalspent(self):
        x = 0
        for a in range(self.autoclick2):
            x += round(float(20 * math.pow(1.15, a)), 2)
        for b in range(self.printmoney2):
            x += round(float(375 * math.pow(1.25, b)), 2)
        for c in range(self.counterfeit2):
            x += round(float(9001 * math.pow(1.3, c)), 2)
        for d in range(self.sharecrash2):
            x += round(float(42000 * math.pow(1.4, d)), 2)
        for e in range(self.bankheist2):
            x += round(float(175000 * math.pow(1.1, e)), 2)
        return x
